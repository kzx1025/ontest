...
getAttribute("name")+"老师"%></a></li>
	...
	<ul class="nav nav-pills nav-stacked span3" style="position:fixed">
				...
				<li class="active"><a href="teacher/home4.jsp">创建试卷</a></li>
				<li ><a href="teacher/home5.jsp">查看学生成绩</a></li>

	</ul>
</div>
<div class="span8">
<form  action="teacher/submit_exam.jsp"  method='post' >
		<input type="submit" class="btn btn-large btn-info" value="生成试卷" name='produce'/>
		<h2>请输入试卷名称</h2>
		<input type="text" name='exam_name' class='span10'  id='exam_name'/>
		<h2>请输入试卷id(输入数字)</h2>
		<input type="text" name='exam_id' class='span10'  id='exam_id'/>
	<p></p>
	<table cellpadding="10" border="1">
	<tr><td>题目id</td><td>题目内容</td><td>答案</td><td>勾选</td></tr>
	<%
...
{ 
 out.println("<tr>");
   ...
   out.println("<td>"+rs.getString("context")+"</td>");
    ...
    %>
     <td>
    <input  id="flag" type="checkbox" name="flag" value="<%out.print(rs.getString("questionid"));%>" style="width: 20px; height: 20px;margin-top: 10px;"/>
    </td>
    </tr>
<%
}
...
e.printStackTrace();
	...
	if((String) session.getAttribute("message")=="success")
	{
	 String show = "<script language='javascript' type='text/javascript'>alert('提交试卷成功,你可以在查看试卷页中发现')</script>";
  out.println(show);
  message="";
session.setAttribute("message",message);
	}
		if((String) session.getAttribute("message")=="false")
	{
	 String show = "<script language='javascript' type='text/javascript'>alert('提交试卷失败')</script>";
  out.println(show);
  message="";
session.setAttribute("message",message);
	}
